...
#
# Author - Adi Zohar, Feb 28th 2020, Amended Jul 1st 2025 with FOCUS
#
# Run Multi daily focus load for crontab use
#
# Amend variables below and database connectivity
# Use .oci/config profiles with user authentications
#
# Crontab set:
# 0 0 * * * timeout 6h /home/opc/focus_reports_to_adw/shell_scripts/run_multi_daily_focus2adw.sh > /home/opc/focus_reports_to_adw/cron_run_multi_tenants_crontab_run.txt 2>&1
#############################################################################################################################
...
# Check if focus2adw.py already running
if (( `ps -ef |grep python |grep focus2adw.py |wc -l` > 0 ))
then
    echo "focus2adw.py is already running, abort.."
    ps -ef |grep python |grep focus2adw.py
    exit 1
fi
...
run_report()
{
    ...
    python3 $APPDIR/focus2adw.py $tenant -du $DATABASE_USER -ds $DATABASE_SECRET_ID -dst $DATABASE_SECRET_TENANT -dn $DATABASE_NAME -d $EXTRACT_DATE -ts1 "${TAG1}" -ts2 "${TAG2}" -ts3 "${TAG3}" -ts4 "${TAG4}" $6 |tee -a $OUTPUT_FILE
