...
resource oci_core_network_security_group_security_rule nsg_rule_1 {
  count                     = var.autonomous_pe_enabled ? 1 : 0
	description               = "Network Security Group for Focus2ADW Ingress port 1522"
	destination_type          = ""
	direction                 = "INGRESS"
...
resource oci_core_network_security_group_security_rule nsg_rule_2 {
  count                     = var.autonomous_pe_enabled ? 1 : 0
	description               = "Network Security Group for Focus2ADW Ingress port 443"
	destination_type          = ""
	direction                 = "INGRESS"
	network_security_group_id = oci_core_network_security_group.vcn_nsg[count.index].id
	protocol                  = "6"
...
resource oci_core_network_security_group_security_rule nsg_rule_3 {
  count                     = var.autonomous_pe_enabled ? 1 : 0
	description               = "Network Security Group for Focus2ADW Egress"
...
resource "oci_load_balancer_backend" "focus2adw_backend" {
	  count            = var.load_balancer_enabled ? 1 : 0
    backendset_name  = oci_load_balancer_backend_set.focus2adw_backendset[count.index].name
    ip_address       = var.adw_pe_ip_address
    load_balancer_id = oci_load_balancer_load_balancer.focus2adw_loadbalancer[count.index].id
    port = 443
}

resource "oci_load_balancer_backend_set" "focus2adw_backendset" {
  	count            = var.load_balancer_enabled ? 1 : 0
    load_balancer_id = oci_load_balancer_load_balancer.focus2adw_loadbalancer[count.index].id
    name             = "focus2adw_backendset"
    policy           = "ROUND_ROBIN"
    health_checker {
        protocol = "TCP"
        interval_ms = 10000
...
resource "oci_load_balancer_listener" "focus2adw_listener" {
	  count            = var.load_balancer_enabled ? 1 : 0
    load_balancer_id = oci_load_balancer_load_balancer.focus2adw_loadbalancer[count.index].id
    name             = "focus2adw_listener"
    port             = 443
    protocol         = "TCP"
    default_backend_set_name = oci_load_balancer_backend_set.focus2adw_backendset[count.index].name
...
resource "oci_load_balancer_load_balancer" "focus2adw_loadbalancer" {
	  count              = var.load_balancer_enabled ? 1 : 0
    compartment_id     = var.compartment_id
...
output "load_balancer_ip_address" {
  value = var.load_balancer_enabled && length(oci_load_balancer_load_balancer.focus2adw_loadbalancer) > 0 ? oci_load_balancer_load_balancer.focus2adw_loadbalancer[0].ip_address_details[0].ip_address : null
}
