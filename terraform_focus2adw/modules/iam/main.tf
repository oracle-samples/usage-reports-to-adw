...
resource "oci_identity_dynamic_group" "dynamic_group" {
    ...
    description    = "Focus2ADW_Dynamic_Group to define the Focus2ADW Compute VM"
...
resource "oci_identity_policy" "policy" {
    count          = var.iam_enabled ? 1 : 0
    compartment_id = var.tenancy_ocid
    description    = "Focus2ADW Policy to allow the VM to extract Focus Report and list compartments"
    name           = var.policy_name
    statements     = [
        "define tenancy focus-report as ocid1.tenancy.oc1..aaaaaaaaned4fkpkisbwjlr56u7cj63lf3wffbilvqknstgtvzub7vhqkggq",
        "endorse dynamic-group ${var.dynamic_group_name} to read objects in tenancy focus-report",
